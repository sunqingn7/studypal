...
const renderTreeNode = (node: TreeNode, index: number, depth: number = 0, parentPath: number[] = []) => {
    ...
    return (
      <div key={node.item.path} className="mb-0.5">
        ...
        {/* Icon */}
        <span className="w-3 h-3 flex items-center justify-center flex-shrink-0 mr-1.5">
          {node.item.type === 'directory' ? (
            node.isExpanded ? (
              <FolderOpen className="w-3 h-3 text-[var(--folder-open-color)]" />
            ) : (
              <Folder className="w-3 h-3 text-[var(--folder-color)]" />
            )
          ) : (
